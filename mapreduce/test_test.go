...
func port(portNum string) string {
	return "localhost:" + portNum
}
...
func TestParallelBasic(t *testing.T) {
	mr := setup()
	for i := 0; i < 2; i++ {
		go RunWorker(mr.address, port(strconv.Itoa(i + 7778)),
...
func TestOneFailure(t *testing.T) {
	...
	go RunWorker(mr.address, port(strconv.Itoa(7778)),
		MapFunc, ReduceFunc, 10, nil)
	go RunWorker(mr.address, port(strconv.Itoa(7779)),
		MapFunc, ReduceFunc, -1, nil)
	mr.Wait()
	check(t, mr.files)
	checkWorker(t, mr.stats)
